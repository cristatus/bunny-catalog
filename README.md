...
- **JVM ecosystem.** JDK distros (Temurin, Corretto, GraalVM, Liberica, Zulu, OpenJ9/Semeru, JetBrains Runtime), build tools (Maven, Maven Daemon, Gradle, sbt, Ant, JBang), language compilers (Kotlin, Scala), framework CLIs (Micronaut, Quarkus, Spring Boot), and diagnostics tools (JMC, VisualVM, async-profiler, Arthas).
...
├── editor/{id}/manifest.yaml    # VS Code, Cursor, Zed, Neovim, ...
├── ide/{id}/manifest.yaml       # JetBrains Toolbox, Eclipse, NetBeans
